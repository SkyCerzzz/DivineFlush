...
local DFTile = assert(SMODS.load_file("src/settings/tile.lua"))()
local map = DF.list_utils.map
          ...
          second_row,
          {
            ...
            nodes = {
              create_option_cycle({
                ...
                opt_callback = "DF_update_config_page",
                scale = 0.8,
                colour = G.C.RED,
...
function G.FUNCS.DF_update_config_page(e)
  if not e or not e.cycle_config then return end

  local grid_wrap = G.OVERLAY_MENU:get_UIE_by_ID("DF_grid_wrap")
...
function SMODS.current_mod.config_tab()
  ...
  return {
    ...
    nodes = {
      { n = G.UIT.O, config = { id = "DF_grid_wrap", object = grid } }
    }
  }
end
