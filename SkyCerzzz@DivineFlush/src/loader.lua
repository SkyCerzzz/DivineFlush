  ...
  if file.list then
    for _, item in ipairs(file.list) do
      if file.config_key then
        item.DF_config_key = file.config_key
        if not divineflush_config[item.DF_config_key] then
...
local function load_pokemon(item)
  local custom_prefix = item.DF_inject_prefix or "DF"

  local custom_atlas = item.atlas and string.find(item.atlas, "DivineFlush")
...
local function load_pokemon_family(file)
  local names = DF.list_utils.map(file.list, function(a) return a.name end)
  if file.family then
...
function pokermon.load_pokemon(item)
  if item.DF_inject_prefix then
    item.key = item.DF_inject_prefix .. '_' .. item.name
